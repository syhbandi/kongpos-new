...
import satuanColumns from "../../../constants/ColumnsHelper/satuan";
import Pagination from "../../../components/Table/Pagination";

const index = () => {
  const company_id = useRecoilValue(companyIdState);
  const { access_token } = useRecoilValue(userState);
  const [params, setParams] = useState<GetSatuansType>({
    company_id,
    length: 10,
    limit: 0,
    order_col: "",
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getSatuans(params, access_token),
  });
  return (
    <>
      <h1 className="text-2xl font-poppins font-semibold mb-5">Satuan</h1>
