...
const Edit = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["satuan", kd_satuan],
    queryFn: () =>
      getSatuan({ company_id, kd_satuan: kd_satuan || "" }, access_token),
  });
  console.log(data);

  const queryClient = useQueryClient();
  const mutation = useMutation({
    mutationFn: updateSatuan,
    onSuccess: () => {
      toast.success("Berhasil memperbarui satuan");
      queryClient.invalidateQueries({ queryKey: ["satuan"] });
      navigate("/dashboard/satuan", { replace: true });
    },
    onError: () => {
  ...
  return (
    <>
      <div className="flex items-center mb-5">
        ...
        />
        <h1 className="text-2xl font-semibold font-poppins">Edit Satuan</h1>
      </div>
      <div className="rounded shadow bg-white p-5 w-full md:w-1/2 lg:w-2/5">
        <FormProvider {...methods}>
          ...
          >
            <Input label="kode" name="kd_satuan" disabled={true} />
            <Input label="nama" name="nama" placeholder="Nama Satuan" />
            <Input
              label="keterangan"
              name="keterangan"
