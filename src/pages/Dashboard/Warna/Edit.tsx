...
const Edit = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["warna", kd_warna],
    queryFn: () =>
      getWarna({ company_id, kd_warna: kd_warna || "" }, access_token),
  });
  console.log(data);

  const queryClient = useQueryClient();
  const mutation = useMutation({
    mutationFn: updateWarna,
    onSuccess: () => {
      toast.success("Berhasil memperbarui warna");
      queryClient.invalidateQueries({ queryKey: ["warna"] });
      navigate("/dashboard/warna", { replace: true });
    },
    onError: () => {
  ...
  return (
    <>
      <div className="flex items-center mb-5">
        ...
        />
        <h1 className="text-2xl font-semibold font-poppins">Edit Warna</h1>
      </div>
      <div className="rounded shadow bg-white p-5 w-full md:w-1/2 lg:w-2/5">
        <FormProvider {...methods}>
          ...
          >
            <Input label="kode" name="kd_warna" disabled={true} />
            <Input label="nama" name="nama" placeholder="Nama Warna" />
            <Input
              label="keterangan"
              name="keterangan"
