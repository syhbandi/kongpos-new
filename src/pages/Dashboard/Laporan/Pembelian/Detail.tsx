...
import { exportPembelian, getPembelian } from "../../../../api/laporan";
...
const Detail = ({ awal, akhir, jenis }: Props) => {
  ...
  const queries = useQueries({
    queries: [
      {
        queryKey: ["pembelian", params],
        queryFn: () => getPembelian(params, access_token),
      },
      {
        queryKey: ["pembelian", { ...params, count_stats: 1 }],
        queryFn: () =>
          getPembelian({ ...params, count_stats: 1 }, access_token),
      },
    ],
  });

  const exportMutation = useMutation({
    mutationFn: exportPembelian,
    onSuccess(data) {
      fileDownload(data, `Pembelian:${params.awal}:${params.akhir}.xlsx`);
    },
  });
