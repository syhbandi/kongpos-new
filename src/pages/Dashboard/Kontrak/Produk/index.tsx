...
import {
  ProdukKontrakCountType,
  ProdukKontrakParams,
  ProdukKontrakType,
} from "../../../../constants/Types/kontrakTypes";
import { SortingState } from "@tanstack/react-table";
import { useQueries } from "@tanstack/react-query";
import { getProdukKontrak } from "../../../../api/kontrak";
import Table from "../../../../components/Table";
import produkKontrakColumns from "../../../../constants/ColumnsHelper/kontrak/produkKontrak";
...
const index = () => {
  ...
  const [data, setData] = useState<ProdukKontrakType[]>([]);
  const [count, setCount] = useState<ProdukKontrakCountType>({
    "Jumlah Record": "0",
  });
  const [sorting, setSorting] = useState<SortingState>([]);
  const [params, setParams] = useState<ProdukKontrakParams>({
    comp_id: companyId,
    count_stats: 0,
    length: 10,
    limit: 0,
  ...
  const queries = useQueries({
    queries: [
      {
        queryKey: ["produkKontrak", params],
        queryFn: () => getProdukKontrak(params, access_token),
      },
      {
        queryKey: ["produkKontrak", { ...params, count_stats: 1 }],
        queryFn: () =>
          getProdukKontrak({ ...params, count_stats: 1 }, access_token),
      },
    ],
  });
